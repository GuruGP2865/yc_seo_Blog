...
import { uploadFilterImage } from "../../../actions/admin/posts";
import { editCategory, createCategory } from "../../../actions/admin/category";
...
export default function CategoryCard({ type, serial, id, name, image}) {
    ...
    return (
      ...
      >
        <div style={modalStyle} className={classes.paper}>
          ...
          <img className="preview__image" src={`${BACKEND}uploads/${newImage}`} />
          <input name="photo" className="file__upload__button" type="file" onChange={(e) => handleImage(e)} /> 
          <Button onClick={
            () => {
              if (newValue != '') {
                return editCategory(id, newValue, newImage).then(data => {
                  if ( data.error) {
      ...
      >
        <div style={modalStyle} className={classes.paper}>
          ...
          <Button onClick={
            () => {
              if (create != '') {
                return createCategory(create, newImage).then(data => {
                  if ( data.error) {
  ...
  return (
    <React.Fragment>
      ...
      <Fab color="primary" aria-label="add" className={classes.addIcon}>
        <IconButton aria-label="View" onClick={handleImputCreate}>
          <AddIcon />
        </IconButton>
      </Fab>
      {editSingleValue()}
